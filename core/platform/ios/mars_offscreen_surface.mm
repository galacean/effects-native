//
//  mars_offscreen_surface.m
//  Mars
//
//  Created by changxing on 2022/8/25.
...
public:
    ...
    bool Init(void* shared_egl_context) {
        EAGLContext* shareContext = (__bridge EAGLContext*) shared_egl_context;
        context_ = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES3 sharegroup:shareContext.sharegroup];
        gles_version_ = 3;
        if (!context_) {
            context_ = [[EAGLContext alloc] initWithAPI:kEAGLRenderingAPIOpenGLES2 sharegroup:shareContext.sharegroup];
            gles_version_ = 2;
        }
        return (context_ != nil);
    }
...
bool MarsOffscreenSurface::Create(const SystemSurface &system_surface, int32_t s_width, int32_t s_height) {
    if (s_width <= 0 || s_height <= 0) {
        MLOGE("invalid surface size: %d %d", s_width, s_height);
        return false;
    }

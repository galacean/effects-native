...
const std::string ANIMATION_GLSL = R"(
...
mat4 getJointNormalMatrix(float boneNdx) {
    ...
    float v = (boneNdx + 0.5) / float(JOINT_COUNT);
    return mat4(
        texture2D(u_jointNormalMatrixSampler, vec2(ROW0_U, v)),
        texture2D(u_jointNormalMatrixSampler, vec2(ROW1_U, v)),
        texture2D(u_jointNormalMatrixSampler, vec2(ROW2_U, v)),
        texture2D(u_jointNormalMatrixSampler, vec2(ROW3_U, v))
    );
    #else
